...
var FraglateClass = (function () {
  ...
  function __is_exist_locale(locales, lang, region) {
    if (_.isNil(region) && !_.isNil(locales[lang])) {
      return true;
    } else if (!_.isNil(region) && !_.isNil(locales[lang]) && !_.isNil(_.get(locales, [lang, 'region', region]))) {
      return true;
    }
    return false;
  }
  ...
  Fraglate.prototype.debug_log = function (log_str) {
    var self = this;
        ...
        _.map(
          ...
          function (filename) {
            return fs.readFileAsync(path.join(locale_path, filename))
              .then(function (content) { return [filename, content]; })
  ...
  Fraglate.prototype._e = function (str, locale_str, context) {
    var self = this;
    if (_.isNil(locale_str)) {
      locale_str = (!_.isNil(self.express_request) ? self.express_request.locale : self.config.default_locale);
    }
    var lang_region = __get_lang_region(locale_str);
    var lang = lang_region[0];
    var region = lang_region[1];
  ...
  Fraglate.prototype.translate = function (key, lang, region, context) {
    ...
    var localeunit = (_.isNull(region) ? _.get(self.locales, [lang, 'data', key]) : _.get(self.locales, [lang, 'region', region, key]));
    if (typeof (localeunit) === 'string') {
      ...
      console.error('locale: ' + __construct_lang_region(lang, region) +
        ...
        key.toString() + '"');
      self.debug_log("locale map for '" + __construct_lang_region(lang, region) + "': ");
      self.debug_log(JSON.stringify(_.get(self.locales, [lang])));
      return null;
    } else {
      console.error('locale: ' + __construct_lang_region(lang, region) +
        ", translation to '" + (typeof localeunit).toString() + "' is not supported!");
      return null;
    }
  };
